...
export default function UsersTable() {
    ...
    useEffect(() => {
        async function getUsers() {
            ...
            const rows = results.map((user) => (createData(
                user._id,
                user.email,
                user.username,
                user.access_rights['read'] + "," + user.access_rights['write'] + "," + user.access_rights['edit'],
                user.is_active,
                user.score
            )))
            setRows(rows)
        ...
        return (
            <TableContainer component={Paper}>
                <Table className={classes.table} aria-label="customized table">
                    ...
                    <TableBody>
                        {rows.map((row) => (<UsersTableRow rows={rows} row={row} setRows={setRows}/>))}
                    </TableBody>
                </Table>
            </TableContainer>
        );
